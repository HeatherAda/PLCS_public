...
def loadDec_Feat_Turk(dataset, filepath):
    if dataset == 'proton-beam':
        ...
        # [rec_nums, texts] = zip(*pub_dic.items())
        (turk_dic, rel_dic) = get_turk_data(
            dataset)  # [Response] turk_dic: {abstract ID: [(Q3,Q4)]}; rel_dic: {abstract, label}
        texts = []
...
def loadResponse(dataset, filepath):
    ...
    for row in csv_reader:
        ...
        # oracles
        if WorkerId not in all_oracles:
            all_oracles[WorkerId] = 1
        else:
            all_oracles[WorkerId] += 1
        # instances
        if AbstractId not in all_instances:
            all_instances[AbstractId] = 1
        else:
            all_instances[AbstractId] += 1
...
def responseOut2In(responses, oracle_idx, instance_idx):
    inside_response = {
    instance_idx[abstractID]: {oracle_idx[oid]: responses[abstractID][oid] for oid in responses[abstractID]} for
    abstractID in responses}
...
if __name__ == '__main__':
    ...
    print("size: ", len(select_instanceid), "oracleid: ", max_oi, "label distri: ", label_num, "oracle quality: ",
          acc_per_class)
